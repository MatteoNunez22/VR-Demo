...
public class NiwController : ReceiveOscBehaviourBase
{
    ...
    public GameObject sendWaterControllerObject;
    private OscSendController m_SendWaterController;
    ...
    public GameObject GranularSurface;
    ...
    public override void Start ()
    {
        ...
        OscSendController waterController = sendWaterControllerObject.GetComponent<OscSendController>();

        if (waterController == null)
        {
            Debug.LogError(string.Format("The GameObject with the name '{0}' does not contain a OscSendController component", sendWaterControllerObject.name));
            return;
        }

        m_SendWaterController = waterController;
    ...
    void Update () {
        ...
        if (!WiimoteManager.HasWiimote()) {
            ...
            Debug.Log("No WiimoteManager_1");

            WiimoteManager.FindWiimotes();

            Debug.Log("No WiimoteManager_2");
            ...
            Debug.Log("Checking key pressing ends.");
        }
        else
        {
        ...
        // update height map shader position
        GranularSurface.GetComponent<Kvant.Stream>().cavePosition = transform.position;
    }
    ...
    protected override void ReceiveMessage(OscMessage message) {
        ...
        else if (message.Address.Equals("/niw/client/aggregator/floorcontact"))
        {
            ...
            if (((string)message[0]).Equals("add"))
            {
                ...
                GranularSurface.GetComponent<GranularSurfaceController>().ApplyStep(new Vector3(x, 0, z) + transform.position);
            }
            ...
            else if (objectUnderFoot == WaterObject)
            {
                ...
                var arg = new object[4 * 6 * 6];
                for(int i = 0; i < 4 * 6 * 6; i++)
                {
                    if (i < 4) arg[i] = (int)100000;
                    else arg[i] = (int)0;
                }
                SendWater(new OscMessage("/niw/client", arg));
            }
            else
            {
                hapticDebugObjects[id].GetComponent<HapticDebugController>().SetTexture(HapticTexture.None);
    ...
    public void SendWater(OscMessage msg)
    {

        if (m_SendWaterController != null)
        {
            // Send the message
            m_SendWaterController.Sender.Send(msg);
            Debug.Log(msg);
        }
    }
    ...
    {
        camera.projectionMatrix = MakeFrustum((l - x) / 256,
                                              (r - x) / 256,
                                              (b - y) / 256,
                                              (t - y) / 256,
                                              (n - z) / 256,
